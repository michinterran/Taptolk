...
-- QR-only issuance is a Super Admin-owned operation. The request itself is
-- the command that starts the durable generation handoff; it does not create
...
begin
  ...
  while remaining > 0 loop
    ...
    insert into public.qr_batches (
      ...
      'GENERATION_APPROVED', actor_user_id, actor_user_id, now(),
      actor_user_id, now(), gen_random_uuid(), 'QR_ONLY'
    ...
    insert into public.qr_generation_jobs (
      tenant_id, management_company_id, site_id, qr_batch_id, approval_request_id
    ) values (
      created_batch.tenant_id, created_batch.management_company_id,
      created_batch.site_id, created_batch.id, gen_random_uuid()
...
-- Release QR-only requests created by the previous two-step implementation.
-- The requester's identity is retained as the Super Admin command actor for
-- the new policy, and each Batch receives one durable generation job.
do $$
declare
  pending_batch public.qr_batches%rowtype;
begin
  for pending_batch in
    select batch.*
    from public.qr_batches as batch
    where batch.request_mode = 'QR_ONLY'
      and batch.status = 'FINAL_APPROVAL_PENDING'
    order by batch.created_at, batch.id
    for update
  loop
    if not exists (
      select 1 from public.qr_generation_jobs as job
      where job.tenant_id = pending_batch.tenant_id
        and job.qr_batch_id = pending_batch.id
    ) then
      insert into public.qr_generation_jobs (
        tenant_id, management_company_id, site_id, qr_batch_id, approval_request_id
      ) values (
        pending_batch.tenant_id, pending_batch.management_company_id,
        pending_batch.site_id, pending_batch.id, gen_random_uuid()
      );
    end if;

    update public.qr_batches
    set status = 'GENERATION_APPROVED',
        generation_approved_by = pending_batch.requested_by,
        generation_approved_at = coalesce(pending_batch.generation_approved_at, now())
    where id = pending_batch.id;

    insert into public.audit_logs (
      tenant_id, site_id, actor_type, actor_id, action, resource_type,
      resource_id, before_data, after_data, reason, request_id
    ) values (
      pending_batch.tenant_id, pending_batch.site_id, 'SYSTEM', pending_batch.requested_by,
      'QR_ONLY_GENERATION_POLICY_MIGRATED', 'QR_BATCH', pending_batch.id,
      jsonb_build_object('batchStatus', 'FINAL_APPROVAL_PENDING'),
      jsonb_build_object('batchStatus', 'GENERATION_APPROVED', 'policy', 'SUPER_ADMIN_DIRECT'),
      'QR-only generation no longer requires a second Super Admin confirmation.',
      gen_random_uuid()
    );
  end loop;
end;
...
comment on function public.request_admin_qr_only_generation(uuid, integer, integer, text, uuid) is
  'Super Admin QR-only generation command. The request atomically creates the Batch and durable generation job; no second approval is required.';
